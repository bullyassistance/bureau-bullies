...
def analyze_report(
    ...
    try:
        ...
        logger.error("Bully AI returned invalid JSON: %s", raw[:400])
        raise RuntimeError(f"Bully AI returned invalid JSON: {e}") from e

    violations = [Violation(**v) for v in data.pop("violations", [])]
    summary = ReportSummary(violations=violations, **data)
    logger.info(
